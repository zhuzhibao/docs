...
var config = {
  method: 'delete',
  url: 'http://local.build-dream.cn/api/administrator/menu/168',
  headers: { 
...
{% tab title="cURL" %}
```bash
curl --location --request DELETE 'http://local.build-dream.cn/api/administrator/menu/168' \
...
{% tab title="HTTP" %}
```http
DELETE /api/administrator/menu/168 HTTP/1.1
